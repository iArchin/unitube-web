...
const Sidebar = ({ className }: Props) => {
  return (
    <ScrollArea
      className={cn(
        "h-screen w-60 pt-3 z-40 !fixed hidden md:block top-0 dark:bg-[#111111] dark:text-white bg-white text-black -translate-x-full transition-transform duration-500 border-r border-[#fdc6003f]",
        className
      )}
    >
      <div className="px-5 flex items-center">
        <Link href="/" className="flex items-center space-x-2">
          ...
          <span className="text-2xl font-black">UniTube</span>
        </Link>
      </div>
      <div className="space-y-4 py-4">
        ...
        <div className="px-3 py-2">
          ...
          <div className="space-y-1">
            ...
            <Button variant="ghost" className="w-full justify-start font-light">
              <MonitorPlay size={24} className="mr-3" />
              Your videos
            </Button>
            <Button variant="ghost" className="w-full justify-start font-light">
              <Clock4 size={24} className="mr-3" />
              Watch later
            </Button>
          </div>
        </div>
